export function LinkedInNavbar() {
  return (
    <nav className="w-full bg-white border-b border-gray-300 flex items-center px-2 z-50 sticky top-0 min-h-[56px] shadow-sm">
      ...
      <div className="flex items-center gap-2 flex-shrink-0">
        ...
        <div className="ml-2 bg-[#edf3f8] rounded flex items-center px-2 w-[280px] h-9 border border-gray-300 focus-within:ring-2 focus-within:ring-[#0A66C2]">
          ...
          <input className="bg-transparent outline-none text-[15px] w-full text-gray-800 placeholder-gray-500" placeholder="Search" />
        </div>
      </div>
      {/* Center: Nav Icons */}
      <div className="flex-1 flex justify-center gap-8 ml-8">
        ...
        <NavIcon icon={<UserIcon />} label="Me" avatarUrl="https://randomuser.me/api/portraits/men/32.jpg" dropdown />
      </div>
      {/* Right: For Business, Premium */}
      <div className="flex items-center gap-6 ml-8">
        <div className="flex flex-col items-center text-xs text-gray-700 cursor-pointer hover:text-black border-r pr-6">
          <GridIcon />
          <span className="mt-0.5 font-normal whitespace-nowrap">For Business <span className="text-[10px]">▼</span></span>
        </div>
        <div className="flex flex-col items-center text-xs cursor-pointer">
          ...
          <span className="mt-0.5 text-[#915907] font-semibold whitespace-nowrap">Try Premium for ₹0</span>
        </div>
      </div>
    </nav>
  );
...
function NavIcon({ icon, label, badge, count, avatarUrl, active, dropdown }: { icon?: React.ReactNode; label: string; badge?: boolean; count?: number; avatarUrl?: string; active?: boolean; dropdown?: boolean }) {
  return (
    <div className={`flex flex-col items-center justify-center relative cursor-pointer w-16 group ${active ? 'text-black' : 'text-gray-700 hover:text-black'}`}> 
      <div className="relative flex items-center justify-center">
        ...
        {badge && (
          <span className="absolute -top-2 -right-2 bg-red-600 text-white text-xs rounded-full px-1.5 min-w-[18px] text-center font-bold border-2 border-white shadow">{count || ''}</span>
        )}
      </div>
      <span className={`text-[12px] mt-1 leading-tight font-normal ${active ? 'font-bold' : ''}`}>{label} {dropdown && <span className="text-[10px]">▼</span>}</span>
    </div>
  );
}
